...
def init_seg_needed(img, image_title, compactness, n_segments):
    '''
    Compares input values for initial segmentation to the ones of the previous one,
    and determines whether a new one needs to be made.
    '''

    if not os.path.isfile('last_seg.npy'):
        return True

    last_seg = np.load('last_seg.npy')

    if image_title != last_seg[0]:
        return True

    if compactness != last_seg[1]:
        return True

    if n_segments != last_seg[2]:
        return True

    return False

def save_init_seg(labels,img,image_title,compactness,n_segments):
    '''
    ...
    last_seg[0] = image_title
    last_seg[1] = compactness
    last_seg[2] = n_segments

    np.save('labels', labels)
    np.save('regions', mean_color)
    np.save('last_seg', last_seg)


def make_image(w,h,initColor,newColor,name):
    '''
    Makes a two-color image of chosen size.
    '''
